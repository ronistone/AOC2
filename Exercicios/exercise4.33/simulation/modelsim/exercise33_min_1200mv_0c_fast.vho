...
ARCHITECTURE structure OF exercise33 IS
SIGNAL gnd : std_logic := '0';
SIGNAL vcc : std_logic := '1';
SIGNAL unknown : std_logic := 'X';
...
-- Equation(s):
-- \Selector0~0_combout\ = (\state.S0~q\ & (\b~input_o\ & ((\a~input_o\) # (\state.S1~q\)))) # (!\state.S0~q\ & (((\a~input_o\))))
...
-- Equation(s):
-- \Selector1~0_combout\ = (\b~input_o\ & ((\state.S1~q\) # ((\a~input_o\ & \state.S2~q\))))
...
-- Equation(s):
-- \q~0_combout\ = (\state.S2~q\ & (\b~input_o\ & \a~input_o\))
